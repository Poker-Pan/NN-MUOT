...
class Loss_Net(nn.Module):
    ...
    def forward(self, nodes, elements):
        ...
        def compute_change_P(self, nodes):
            ...
            elif type_surface == 'Cylinder' or type_surface == 'Opener' or type_surface == 'Hand' or type_surface == 'Airplane' or type_surface == 'Mazes' \
            ...
            product = torch.bmm(norm.unsqueeze(2), norm.unsqueeze(1))
            ...
            def initial_end_data(nodes, elements):
                ...
                if self.Key_Para['type_node'] == 'Load':
                    ...
                    if type_surface == 'Cylinder':
                        ...
                        rho_initial = Sphere_Surface_Gaussian(nodes, elements, self.Key_Para['mu_0'], self.Key_Para['sigma_0'])
                        # rho_initial = rho_initial + 0.5*Sphere_Surface_Gaussian(nodes, elements, np.array([0.0, 0.5, 0.5]), self.Key_Para['sigma_0'])
                        # rho_initial = rho_initial + 0.5*Sphere_Surface_Gaussian(nodes, elements, np.array([0.5, 0.0, 0.5]), self.Key_Para['sigma_0'])
                        # rho_initial = rho_initial + 0.5*Sphere_Surface_Gaussian(nodes, elements, np.array([0.5, 1.0, 0.5]), self.Key_Para['sigma_0'])
                        rho_end = 0.5*Sphere_Surface_Gaussian(nodes, elements, self.Key_Para['mu_1'], self.Key_Para['sigma_1'])
                        rho_end = rho_end + 0.5*Sphere_Surface_Gaussian(nodes, elements, np.array([0.0, 0.5, 0.5]), self.Key_Para['sigma_1'])
                        rho_end = rho_end + 0.5*Sphere_Surface_Gaussian(nodes, elements, np.array([0.5, 0.0, 0.5]), self.Key_Para['sigma_1'])
                        rho_end = rho_end + 0.5*Sphere_Surface_Gaussian(nodes, elements, np.array([0.5, 1.0, 0.5]), self.Key_Para['sigma_1'])
            ...
            if type_surface == 'Cylinder':
                ...
                if 1:
                    ...
                    for i in range(1, T+1):
                        ...
                        ax.yaxis.set_pane_color((1.0, 1.0, 1.0, 1.0))
                        ax.zaxis.set_pane_color((1.0, 1.0, 1.0, 1.0))
                        ax.view_init(elev=id_elev[i-1], azim=85)
                        # ax.view_init(elev=90, azim=45)
                        # ax.set_xlim3d(0, 1), ax.set_ylim3d(0, 1), ax.set_zlim3d(0, 1)
                ...
                if 1:
                    ...
                    for i in range(1, T+1):
                        ...
                        ax.yaxis.set_pane_color((1.0, 1.0, 1.0, 1.0))
                        ax.zaxis.set_pane_color((1.0, 1.0, 1.0, 1.0))
                        ax.view_init(elev=25, azim=10 + 40*(i-1))
                        # ax.view_init(elev=90, azim=45)
                        # ax.set_xlim3d(0, 1), ax.set_ylim3d(0, 1), ax.set_zlim3d(0, 1)
                        # ax.set_xticks([0.0, 0.5, 1.0]),  ax.set_yticks([0.0, 0.5, 1.0]), 
                        ax.set_zticks([cur_nodes[:, 3].min(), cur_nodes[:, 3].max()])
                ...
                if 1:
                    ...
                    for i in range(1, T+1):
                        ...
                        surf = ax.plot_trisurf(cur_nodes[:, 1], cur_nodes[:, 2], cur_nodes[:, 3], triangles=elements, vmin=0, vmax=1, antialiased=False)
                        surf.set_array(color_rho[:, 0])
                        ax.view_init(elev=60, azim=10 + 40*(i-1))
                        # ax.view_init(elev=90, azim=45)
                        # ax.set_xlim3d(0, 1), ax.set_ylim3d(0, 1), ax.set_zlim3d(0, 1)
                        # ax.set_xticks([0.0, 0.5, 1.0]),  ax.set_yticks([0.0, 0.5, 1.0]), 
                        ax.set_zticks([cur_nodes[:, 3].min(), cur_nodes[:, 3].max()])
...
if __name__== "__main__" :
    ...
    if test == 'Cylinder':
        ...
        mu_0, mu_1 = [0.5, 0.5, 1.0], [1.0, 0.5, 0.5]
