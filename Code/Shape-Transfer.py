...
class Loss_Net(nn.Module):
    ...
    def forward(self, nodes):
        ...
        def rho_boundary(nodes):
            def load_data():
                # rho_initial = np.load('./Data_Set/Example_Shape_Data/data_circle.npy').reshape((-1, 1))
                # rho_end = np.load('./Data_Set/Example_Shape_Data/data_triangle.npy').reshape((-1, 1)) 
                rho_initial = np.load('./Artical/Images/circle.npy').reshape((-1, 1))
                rho_end = np.load('./Artical/Images/star.npy').reshape((-1, 1)) 
    ...
    def plot_rho(self, model_rho, ite=''):
        if self.Dim_space == 2:
            ...
            if 1:
                ...
                v_min = np.min((pre_rho.min(), 0))
                v_max = np.max((pre_rho.max(), 1))
                norm = matplotlib.colors.Normalize(vmin=v_min, vmax=v_max)
                for i in range(T):
                    plt.figure(figsize=(6, 6))
                    cur_rho = pre_rho[i::T, :].reshape(num, num)

                    plt.imshow(cur_rho, extent=(np.min(nodes[:, 1]), np.max(nodes[:, 1]), np.min(nodes[:, 2]), np.max(nodes[:, 2])), origin='lower', norm=norm)
                    # plt.xticks([])
                    # plt.yticks([])
                    # plt.colorbar()
                    plt.savefig(self.Key_Para['File_name'] + '/' + 'Fig-rho' + str(i) + '.png', dpi=300, bbox_inches='tight')
                    plt.close()

                rho_initial = np.load('./Artical/Images/circle.npy').reshape((-1, 1))
                rho_end = np.load('./Artical/Images/star.npy').reshape((-1, 1)) 
                plt.figure(figsize=(6, 6))
                plt.imshow(rho_initial.reshape(28, 28), extent=(np.min(nodes[:, 1]), np.max(nodes[:, 1]), np.min(nodes[:, 2]), np.max(nodes[:, 2])), origin='lower', norm=norm)
                plt.savefig(self.Key_Para['File_name'] + '/' + 'Fig-rho-initial.png', dpi=300, bbox_inches='tight')
                plt.close()

                plt.figure(figsize=(6, 6))
                plt.imshow(rho_end.reshape(28, 28), extent=(np.min(nodes[:, 1]), np.max(nodes[:, 1]), np.min(nodes[:, 2]), np.max(nodes[:, 2])), origin='lower', norm=norm)
                plt.savefig(self.Key_Para['File_name'] + '/' + 'Fig-rho-end.png', dpi=300, bbox_inches='tight')
                plt.close()
