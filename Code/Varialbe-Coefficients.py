...
import math, os, time, torch, copy, sys, random, inspect
...
class Loss_Net(nn.Module):
    ...
    def forward(self, nodes):
        ...
        def W2(nodes):
            ...
            cur_g = (self.Key_Para['eta']/2)*cur_phi

            out = 0.5*torch.mean(torch.mul(cur_rho, torch.sum(torch.mul(cur_v, cur_v), dim=1).reshape(-1, 1))) + \
                (1/self.Key_Para['eta'])*torch.mean(torch.mul(cur_rho, torch.mul(cur_g, cur_g)))
            return out
        ...
        def Continuity_Equation(nodes):
            ...
            cur_phi = self.model_phi(nodes)
            cur_v = gradients(cur_phi, nodes)[0][:, 1:]
            cur_g = 0*(self.Key_Para['eta']/2)*cur_phi
            ...
            out = cur_grad_rho_t + div_cur_rho_v.reshape(-1, 1) - cur_rho_g
        ...
        def Hamilton_Jacobi_Equation(nodes):
            ...
            out = grad_cur_phi[:, 0].reshape(-1, 1) + 0.5*torch.sum(torch.mul(grad_cur_phi[:, 1:], grad_cur_phi[:, 1:]), dim=1).reshape(-1, 1) \
                + torch.mul(cur_phi, cur_g) - (1/self.Key_Para['eta'])*torch.mul(cur_g, cur_g)
            out = torch.mean(torch.mul(out, out))
            return out
        ...
        def rho_boundary(nodes):
            ...
            def Gaussian_data():
                rho_initial = 1.0*Gaussian_distribution_torch(self.Key_Para['mu_0'], self.Key_Para['sigma_0'], nodes[::self.Key_Para['Num_Nodes_t'],:])\
                            # + 0.3*Gaussian_distribution_torch(self.Key_Para['mu_0'] + np.array([1.0]), self.Key_Para['sigma_0'], nodes[::self.Key_Para['Num_Nodes_t'],:])
                rho_end = 0.7*Gaussian_distribution_torch(self.Key_Para['mu_1'], self.Key_Para['sigma_1'], nodes[int(self.Key_Para['Num_Nodes_t']-1)::self.Key_Para['Num_Nodes_t'],:])\
                            + 0.3*Gaussian_distribution_torch(self.Key_Para['mu_1'] - np.array([1.0]), self.Key_Para['sigma_1'], nodes[::self.Key_Para['Num_Nodes_t'],:])
                return rho_initial, rho_end
            ...
            nodes_0 = nodes[0:Num_Nodes_t, :]
            nodes_1 = nodes[-Num_Nodes_t:, :]
            cur_rho_0 = self.model_rho(nodes_0)
            cur_rho_1 = self.model_rho(nodes_1)
            out_0 = 1*torch.mean(torch.mul(cur_rho_0, cur_rho_0)) + torch.mean(torch.mul(cur_rho_1, cur_rho_1))
        ...
        loss_rho_ic, loss_rho_bc = rho_boundary(nodes)
        sub_loss = [loss_W2, loss_C_eq, loss_HJ_eq, loss_rho_ic, loss_rho_bc]
        ...
        if self.type_loss == 'Auto-Weight':
            ...
            loss = self.beta[0]*loss_W2 + self.beta[1]*loss_C_eq + self.beta[2]*loss_HJ_eq + self.beta[3]*loss_rho_ic + self.beta[3]*loss_rho_bc
            ...
            for ep_t in range(self.Key_Para['epochs_train']):  
                ...
                if ep_t % 100 ==0:
                    print('ep:', '%d' %ep_t,
                        ...
                        'loss_rho_ic:',"%.6f" %sub_loss[3],
                        'loss_rho_bc:',"%.6f]" %sub_loss[4]
                        )
                    if ep_t <= 4000:
                        self.Key_Para['eta'] = self.Key_Para['eta'] * 1.08
...
class Plot_Result(object):
    ...
    def plot_rho(self, model_rho, ite=''):
        if self.Dim_space == 1:
            ...
            nodes_space = np.linspace(self.Space[0, 0], self.Space[0, 1], num).reshape(-1, 1)
            nodes = np.hstack((t.reshape(-1, 1).repeat(num, axis=1).T.reshape(-1, 1), nodes_space.repeat(T, axis=0)))
            pre_nodes = torch.tensor(nodes, requires_grad=True, dtype=torch.float64).cuda()


            real_rho = torch.zeros((T*num, 1))
            ...
            plt.imshow(pre_rho.reshape(T, num), cmap=cm.jet, 
                        extent=(np.min(nodes[:, 0]), np.max(nodes[:, 0]), np.min(nodes[:, 1]), np.max(nodes[:, 1])), origin='lower')
            plt.xlabel('t')
            plt.ylabel('x')
            plt.colorbar()
            ...
            if 1:
                ...
                for i in range(5):
                    plt.figure(figsize=(6, 6)) #, facecolor='none'
                    plt.plot(nodes_space, pre_rho[i::T], color='b', linewidth=2.0, label='pre_rho')
                    print(np.sum(pre_rho[i::T]*(2/800)))
                    plt.xlim((0, np.max(nodes[:, 1])))
                    plt.ylim((0, 7))
                    plt.xlabel('x')
        ...
        np.save(self.Key_Para['File_name'] + '/' + 'real_rho.npy', real_rho)
    ...
    def plot_v(self, model_phi, ite=''):
        if self.Dim_space == 1:
            ...
            pre_v = pre_v.reshape(-1, 1).cpu().detach().numpy()
 

            plt.figure(figsize=(6, 6))
            plt.imshow(pre_v.reshape(T, num), cmap=cm.jet, 
                        extent=(np.min(nodes[:, 1]), np.max(nodes[:, 1]), np.min(nodes[:, 0]), np.max(nodes[:, 0])), origin='lower')
            plt.title('v')
            plt.colorbar()
            plt.xlabel('x')
            plt.ylabel('t')
    ...
    def plot_g(self, model_phi, ite=''):
        if self.Dim_space == 1:
            ...
            pre_phi = model_phi(pre_nodes)
            pre_g = pre_phi
            pre_g = pre_g.reshape(-1, 1).cpu().detach().numpy()


            plt.figure(figsize=(6, 6))
            plt.imshow(pre_g.reshape(T, num), cmap=cm.jet, 
                        extent=(np.min(nodes[:, 1]), np.max(nodes[:, 1]), np.min(nodes[:, 0]), np.max(nodes[:, 0])), origin='lower')
            plt.title('g')
            plt.colorbar()
            plt.xlabel('x')
...
def main(Key_Para):
    ...
    # model_rho, model_phi = torch.load('20240328-1545-VC-UOT-KKT-1D--2'+ '/model_rho.pth'), torch.load('20240328-1545-VC-UOT-KKT-1D--2'+ '/model_phi.pth')
...
if __name__== "__main__" :
    for j in range(1):
        ...
        if test == 'test1':
            ...
            Dim_space = 1
            Num_Nodes_t = 15
            Num_Nodes_all_space = 800
            mu_0, mu_1 = [0.5], [1.5]
            sigma_0, sigma_1 = 0.005, 0.005
            Time, Space = [0.0, 1.0], [0.0, 2.0] 
            eta = 5/np.power(0.1, 2)
